...
def processwithcomments(caption, instream, outstream, listingslang):
	...
	for line in lines:
		...
		# Remove /// comments
		# old -> line = line.split("///")[0].rstrip()
		for i in range(len(line)-3):
			if (i == 0 or line[i-1].isspace()) and line[i:i+3] == "///" and (i+3 == len(line) or line[i+3].isspace()):
				line = line[:i]
				break
		line = line.rstrip()
...
def processraw(caption, instream, outstream, listingslang = 'raw'):
	try:
		...
		if "Template" in caption:
			hash_script = 'hash'
			p = subprocess.Popen(['sh', 'content/contest/%s.sh' % hash_script], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
			hsh, _ = p.communicate(source.encode())
			hsh = hsh.decode().split(None, 1)[0]
			hsh = hsh + ', '
