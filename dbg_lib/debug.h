...
// Debug prefix
inline void err_prefix(
	const std::string& func, int line, const std::string& args
) {
	std::cerr
...
struct LocalIORedirect {
	LocalIORedirect() {
		if (!freopen(".info/input.txt", "r", stdin)) {
			std::cerr
				<< "\033[1;31mWARNING\033[0m: "
				<< "could not open input.txt\n";
		}
		if (!freopen(".info/output.txt", "w", stdout)) {
			std::cerr
			<< "\033[1;31mWARNING\033[0m: "
			<< "could not open output.txt\n";
		}
	}
};

static LocalIORedirect localIORedirect_;
#else
#define dbg(...) 42
#endif